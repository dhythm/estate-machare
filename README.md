...
不動産の売買・賃貸をつなぐマッチングプラットフォーム。

現在の公開画面は物件写真、間取り・面積・築年・アクセス、売買価格・月額賃料を中心に構成しています。物件の問い合わせから内見や取引条件を相談できます。引越し・運搬サービスは対象外です。旧版の日額レンタル・購入切替データとAPIは互換性のため残しています。以下の旧取引機能に関する記述は、この互換機能を指します。
...
http://localhost:3000 で確認できます。起動時にサンプル（物件 48 件を `lib/server/` で決定的に生成）に加え、数か月使われた後の状態を模したダミーの取引（`lib/server/demo-activity.ts`: 注文・レンタル・問い合わせと返信・レビュー・取引イベント・通知・審査待ちの物件・停止中のアカウント）を投入します。`DEMO_ACTIVITY=off` にするとサンプルの物件だけになります（テストはこの状態で走ります）。物件は作成後に審査待ちとなり、運営が承認すると公開されます。物件への問い合わせ・お問い合わせも保存されます。決済は未実装です。
...
- `lib/server/store/index.ts` が `listings` / `submissions` を `globalThis` 上のシングルトンとして提供します（開発時の HMR で消えません）。テストでは `resetStore()` で初期化します。
- サービス（`lib/server/listings.ts` / `submissions.ts`）はストア経由でのみデータに触れます。
- 連絡先メールアドレスは公開エンティティに保存せず、物件の公開フィールドにも含めません。
- 物件掲載・問い合わせはログインが必要で、作成者の ID を `ownerUserId`（submission は `userId`）として保存します。更新・削除は所有者または運営のみ、審査待ち・却下の行は所有者と運営だけが閲覧できます。
...
| `DEMO_SELLER_EMAIL` / `DEMO_SELLER_PASSWORD` | 出品者役（`user`）のデモアカウント。サンプル出品の一部（trc-001 / cmb-002 など）を所有                        |
| `DEMO_USER_EMAIL` / `DEMO_USER_PASSWORD`     | 購入・賃貸希望者役（`user`）のデモアカウント                                                                  |
...
環境変数のアカウントに加え、`lib/server/auth/accounts.ts` にコード定義の登場人物（物件掲載者・購入希望者・賃貸希望者、いずれも `user` ロール、メールは `<id>@example.com`）があり、サンプル物件・ダミーの取引の相手役になります。出品者ページの名前と出品の所有者はこの人物に対応しています。
...
- `/account`（マイページ）は `lib/server/account.ts` で自分の物件と届いた問い合わせ、送った問い合わせをまとめます。
- 物件への問い合わせはスレッドになります（`lib/server/threads.ts`）。参加者は送信者と物件の所有者で、運営は閲覧のみです。両参加者が返信でき（`messages` テーブル）、対象の所有者が状態（未対応 / 対応中 / 成約 / 見送り）を変えます。
- 既読管理（`lib/server/thread-reads.ts`、`thread_reads` テーブル）: スレッドページを開くと参加者ごとに `readAt` を記録します。所有者は一度も開いていないスレッドと相手の新しい返信、送信者は所有者の新しい返信を未読とし、マイページに「未読」バッジと概要（未読のやり取り、未対応の問い合わせ、申込中のレンタル、審査待ちの出品）を表示します。
...
- 取引の履歴（`lib/server/deal-events.ts`、`deal_events` テーブル）: 注文・レンタルの状態が変わるたびに、誰が・いつ・何に変えたかを記録します（申込・作成、審査、運営の取り消しを含む）。`/account/deals/[kind]/[id]` は当事者と運営だけが開け、要約・時系列・関連するやり取り・関連する取引（レンタルと購入切替の注文）を表示します（`lib/server/deals.ts`）。マイページの「取引の履歴」と運営の注文・レンタル一覧から辿れます。
...
| パス                           | 内容                                                                                                                                      |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `/`                            | トップ。目的別の検索、注目の物件。ログイン済みなら「あなたの状況」（未読・未対応・申込中・審査待ち・通知の件数）                          |
...
| `/account`                     | マイページ。自分の物件と届いた連絡、送った連絡、取引の確認                                                                                |
| `/account/threads/[id]`        | 問い合わせのやり取り（返信、所有者は状態変更）                                                                                            |
| `/account/notifications`       | 通知一覧（クリックで既読にして遷移、すべて既読）                                                                                          |
| `/account/deals/[kind]/[id]`   | 取引の履歴（注文 / レンタルの時系列。当事者と運営のみ）                                                                                   |
| `/admin`                       | 運営画面。ダッシュボード、アカウント管理 `/admin/accounts`、取引管理 `/admin/deals`（掲載審査 / 注文 / レンタル / 問い合わせ / レビュー） |
...
- `lib/server/` は `server-only`。サンプルデータ、検索、取引方法の判定を管理します。
...
- 詳細ページはサーバーでデータを取得します。クライアントには選択状態など表示に必要な処理を残します。
