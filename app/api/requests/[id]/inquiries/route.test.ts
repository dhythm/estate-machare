...
describe('POST /api/requests/[id]/inquiries', () => {
  it('opens a question thread for a request that is not finished', async () => {
    const response = await post('pr-03', { message: '積載方法は？' })
    expect(response.status).toBe(201)
    const [stored] = await listSubmissions('requestInquiry', 'pr-03')
    expect(stored.userId).toBe('demo-user')
    expect(stored.payload.name).toBe('利用者デモ')
    expect((await post('pr-01', { message: '' })).status).toBe(400)
    expect((await post('missing', { message: 'x' })).status).toBe(404)
    await updatePropertyRequestStatus('pr-02', demoSeller, '成約')
    expect((await post('pr-02', { message: 'x' })).status).toBe(409)
    signInAs(null)
    expect((await post('pr-01', { message: 'x' })).status).toBe(401)
  })
})
